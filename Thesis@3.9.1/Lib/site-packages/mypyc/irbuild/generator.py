...
def add_iter_to_generator_class(builder: IRBuilder, fn_info: FuncInfo) -> None:
    ...
    iter_fn_decl = FuncDecl(
        "__iter__", fn_info.generator_class.ir.name, builder.module_name, sig
    )
...
def add_next_to_generator_class(
    ...
    result = builder.add(
        Call(
            fn_decl,
            [self_reg, none_reg, none_reg, none_reg, none_reg],
            fn_info.fitem.line,
        )
    )
    ...
    next_fn_decl = FuncDecl(
        "__next__", fn_info.generator_class.ir.name, builder.module_name, sig
    )
...
def add_send_to_generator_class(
    ...
    result = builder.add(
        Call(
            fn_decl,
            [self_reg, none_reg, none_reg, none_reg, builder.read(arg)],
            fn_info.fitem.line,
        )
    )
    builder.add(Return(result))
    ...
    next_fn_decl = FuncDecl(
        "send", fn_info.generator_class.ir.name, builder.module_name, sig
    )
    next_fn_ir = FuncIR(next_fn_decl, blocks, env)
...
def add_throw_to_generator_class(
    ...
    throw_fn_decl = FuncDecl(
        "throw", fn_info.generator_class.ir.name, builder.module_name, sig
    )
...
def add_close_to_generator_class(builder: IRBuilder, fn_info: FuncInfo) -> None:
    ...
    # Next, add the actual function as a method of the generator class.
    sig = FuncSignature((RuntimeArg(SELF_NAME, object_rprimitive),), object_rprimitive)
    close_fn_decl = FuncDecl(
        "close", fn_info.generator_class.ir.name, builder.module_name, sig
    )
...
def add_await_to_generator_class(builder: IRBuilder, fn_info: FuncInfo) -> None:
    ...
    # Next, add the actual function as a method of the generator class.
    sig = FuncSignature((RuntimeArg(SELF_NAME, object_rprimitive),), object_rprimitive)
    await_fn_decl = FuncDecl(
        "__await__", fn_info.generator_class.ir.name, builder.module_name, sig
    )
...
def setup_env_for_generator_class(builder: IRBuilder) -> None:
    ...
    # Add the type, value, and traceback variables to the environment.
    exc_type = builder.environment.add_local(
        Var("type"), object_rprimitive, is_arg=True
    )
    exc_val = builder.environment.add_local(
        Var("value"), object_rprimitive, is_arg=True
    )
    exc_tb = builder.environment.add_local(
        Var("traceback"), object_rprimitive, is_arg=True
    )
