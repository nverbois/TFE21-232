...
def make_for_loop_generator(
    ...
    if isinstance(expr, CallExpr) and isinstance(expr.callee, RefExpr):
        if (
            expr.callee.fullname == "builtins.range"
            and (
                len(expr.args) <= 2
                or (
                    len(expr.args) == 3
...
class ForGenerator:
    ...
    def load_len(self, expr: Union[Value, AssignmentTarget]) -> Value:
        ...
        return self.builder.builder.builtin_call(
            [self.builder.read(expr, self.line)], "builtins.len", self.line,
        )
...
class ForIterable(ForGenerator):
    ...
    def gen_condition(self) -> None:
        ...
        builder = self.builder
        line = self.line
        self.next_reg = builder.primitive_op(
    ...
    def init(self, expr_reg: Value, target_type: RType, reverse: bool) -> None:
        ...
        if not reverse:
            ...
            index_reg = builder.binary_op(
                self.load_len(self.expr_target), builder.add(LoadInt(1)), "-", self.line
            )
    ...
    def gen_step(self) -> None:
        ...
        builder.assign(
            ...
            builder.primitive_op(
                unsafe_short_add,
                [builder.read(self.index_target, line), builder.add(LoadInt(step))],
                line,
            ),
            line,
        )
    ...
    def gen_step(self) -> None:
        ...
        if is_short_int_rprimitive(self.start_reg.type) and is_short_int_rprimitive(
            ...
            new_val = builder.primitive_op(
                unsafe_short_add,
                [builder.read(self.index_reg, line), builder.add(LoadInt(self.step))],
                line,
            )
...
class ForInfiniteCounter(ForGenerator):
    ...
    def gen_step(self) -> None:
        ...
        new_val = builder.primitive_op(
            unsafe_short_add,
            [builder.read(self.index_reg, line), builder.add(LoadInt(1))],
            line,
        )
        builder.assign(self.index_reg, new_val, line)
        builder.assign(self.index_target, new_val, line)
