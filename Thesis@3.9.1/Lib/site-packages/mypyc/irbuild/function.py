    ...
    if dec.func in builder.nested_fitems:
        ...
        builder.primitive_op(
            ...
            [
                ...
                builder.load_static_unicode(dec.func.name),
                decorated_func,
            ],
            decorated_func.line,
        )
...
def gen_func_item(
    ...
    if fn_info.is_generator:
        add_methods_to_generator_class(
            builder, fn_info, sig, env, blocks, fitem.is_coroutine
        )
    else:
        func_ir, func_reg = gen_func_ir(builder, blocks, sig, env, fn_info, cdef)
...
def handle_ext_method(builder: IRBuilder, cdef: ClassDef, fdef: FuncDef) -> None:
    ...
    if is_decorated(builder, fdef):
        ...
        builder.primitive_op(
            py_setattr_op,
            [typ, builder.load_static_unicode(name), decorated_func],
            fdef.line,
        )
...
def calculate_arg_defaults(
    builder: IRBuilder, fn_info: FuncInfo, env: Environment, func_reg: Optional[Value]
) -> None:
    ...
    for arg in fitem.arguments:
        ...
        if arg.initializer and not is_constant(arg.initializer):
            value = builder.coerce(
                builder.accept(arg.initializer), env.lookup(arg.variable).type, arg.line
            )
...
def handle_yield_from_and_await(
    ...
    def except_body() -> None:
        ...
        res = builder.primitive_op(
            yield_from_except_op, [builder.read(iter_reg)], o.line
        )
