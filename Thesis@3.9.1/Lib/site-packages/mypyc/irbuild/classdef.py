    ...
    for stmt in cdef.defs.body:
        if isinstance(stmt, OverloadedFuncDef) and stmt.is_property:
            ...
            builder.primitive_op(
                py_setattr_op,
                [typ, builder.load_static_unicode(lvalue.name), value],
                stmt.line,
            )
...
def allocate_class(builder: IRBuilder, cdef: ClassDef) -> Value:
    ...
    builder.primitive_op(
        dict_set_item_op,
        [builder.load_globals_dict(), builder.load_static_unicode(cdef.name), tp,],
        cdef.line,
    )
...
def setup_non_ext_dict(
    ...
    has_prepare = builder.primitive_op(
        py_hasattr_op,
        [metaclass, builder.load_static_unicode("__prepare__")],
        cdef.line,
    )
    ...
    builder.assign(
        non_ext_dict, builder.primitive_op(new_dict_op, [], cdef.line), cdef.line
    )
    builder.goto(exit_block)
...
def gen_glue_ne_method(builder: IRBuilder, cls: ClassIR, line: int) -> FuncIR:
    ...
    args = [
        builder.read(builder.environment.add_local_reg(var, type, is_arg=True), line)
    ...
    return FuncIR(
        FuncDecl(
            "__ne__", cls.name, builder.module_name, FuncSignature(rt_args, ret_type)
        ),
        blocks,
        env,
    )
...
def cache_class_attrs(
    builder: IRBuilder, attrs_to_cache: List[Lvalue], cdef: ClassDef
) -> None:
...
def create_mypyc_attrs_tuple(builder: IRBuilder, ir: ClassIR, line: int) -> Value:
    ...
    return builder.primitive_op(
        new_tuple_op, [builder.load_static_unicode(attr) for attr in attrs], line
    )
...
def finish_non_ext_dict(
    ...
    builder.primitive_op(
        dict_set_item_op,
        [non_ext.dict, builder.load_static_unicode("__annotations__"), non_ext.anns],
        -1,
    )
