...
from mypyc.primitives.list_ops import list_extend_op, list_len_op, new_list_op
...
class LowLevelIRBuilder:
    def __init__(self, current_module: str, mapper: Mapper,) -> None:
    ...
    def native_args_to_positional(
        ...
        for lst, arg in zip(formal_to_actual, sig.args):
            ...
            if arg.kind == ARG_STAR:
                output_arg = self.primitive_op(
                    new_tuple_op, [args[i] for i in lst], line
                )
            elif arg.kind == ARG_STAR2:
                dict_entries = [
                    (self.load_static_unicode(cast(str, arg_names[i])), args[i])
                    for i in lst
                ]
    ...
    def make_dict(self, key_value_pairs: Sequence[DictEntry], line: int) -> Value:
        ...
        for key, value in key_value_pairs:
            if key is not None:
                ...
                self.primitive_op(dict_update_in_display_op, [result, value], line=line)
    ...
    def translate_special_method_call(
        ...
        call_c_op = self.matching_call_c(
            call_c_ops_candidates, [base_reg] + args, line, result_type=result_type
        )
        if call_c_op is not None:
            return call_c_op
        return self.matching_primitive_op(
            ops, [base_reg] + args, line, result_type=result_type
        )
