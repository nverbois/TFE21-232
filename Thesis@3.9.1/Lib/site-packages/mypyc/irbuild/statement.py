...
def transform_import(builder: IRBuilder, node: Import) -> None:
    ...
    for node_id, as_name in node.ids:
        ...
        obj = builder.primitive_op(
            dict_get_item_op, [mod_dict, builder.load_static_unicode(base)], node.line
        )
...
def transform_import_from(builder: IRBuilder, node: ImportFrom) -> None:
    ...
    for name, maybe_as_name in node.names:
        ...
        builder.gen_method_call(
            globals,
            "__setitem__",
            [builder.load_static_unicode(as_name), obj],
            result_type=None,
            line=node.line,
        )
...
def transform_try_except(
    ...
    for type, var, handler_body in handlers:
        ...
        if var:
            ...
            builder.assign(
                target, builder.primitive_op(get_exc_value_op, [], var.line), var.line
            )
        handler_body()
...
def transform_with(
    ...
    def finally_body() -> None:
        ...
        builder.add(Branch(builder.read(exc), exit_block, out_block, Branch.BOOL_EXPR))
        builder.activate_block(exit_block)
