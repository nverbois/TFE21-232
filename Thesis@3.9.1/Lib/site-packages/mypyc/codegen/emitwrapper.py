...
def generate_wrapper_function(
    ...
    groups = [
        [arg for arg in real_args if arg.kind == k] for k in range(ARG_NAMED_OPT + 1)
    ]
    reordered_args = (
        groups[ARG_POS] + groups[ARG_OPT] + groups[ARG_NAMED_OPT] + groups[ARG_NAMED]
    )
...
def generate_dunder_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        "static PyObject *{name}({input_args}) {{".format(
            name=name, input_args=input_args,
        )
    )
