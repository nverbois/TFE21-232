    ...
    for r, i in fn.env.indexes.items():
        ...
        declarations.emit_line(
            "{ctype}{prefix}{name}{init};".format(
                ctype=ctype, prefix=REG_PREFIX, name=r.name, init=init
            )
        )
...
class FunctionEmitterVisitor(OpVisitor[None], EmitterInterface):
        ...
        if op.op == Branch.BOOL_EXPR:
            ...
            if isinstance(typ, RTuple):
                ...
                cond = self.emitter.tuple_undefined_check_cond(
                    typ, self.reg(op.left), self.c_error_value, compare
                )
            else:
                cond = "{} {} {}".format(
                    self.reg(op.left), compare, self.c_error_value(typ)
                )
        else:
    ...
    def visit_set_attr(self, op: SetAttr) -> None:
        ...
        if cl.get_method(op.attr):
            ...
            self.emitter.emit_lines(
                "{} = {};".format(attr_expr, src), "{} = 1;".format(dest),
            )
            ...
            if isinstance(op.value, str):
                ...
                self.emitter.emit_line(
                    "PyErr_SetObject(PyExc_{}, {}{});".format(
                        op.class_name, REG_PREFIX, op.value.name
                    )
                )
            else:
