...
class AssignmentTargetTuple(AssignmentTarget):
    ...
    def __init__(
        self, items: List[AssignmentTarget], star_idx: Optional[int] = None
    ) -> None:
        self.items = items
...
class Environment:
    ...
    def add_target(
        self, symbol: SymbolNode, target: AssignmentTarget
    ) -> AssignmentTarget:
...
class Register(Value):
    ...
    def __init__(
        self, type: RType, line: int = -1, is_arg: bool = False, name: str = ""
    ) -> None:
        super().__init__(line)
        self.name = name
        self.type = type
...
)  # To resolve ambiguities, highest priority wins
...
class PrimitiveOp(RegisterOp):
    ...
    def __init__(self, args: List[Value], desc: OpDescription, line: int) -> None:
    ...
    def __repr__(self) -> str:
        return "<PrimitiveOp name=%r args=%s>" % (self.desc.name, self.args)
...
class SetAttr(RegisterOp):
    ...
    def to_str(self, env: Environment) -> str:
        return env.format(
            "%r.%s = %r; %r = is_error", self.obj, self.attr, self.src, self
        )
...
class CallC(RegisterOp):
    ...
    def __init__(
        self, function_name: str, args: List[Value], ret_type: RType, line: int
    ) -> None:
        super().__init__(line)
