        ...
        with use_custom_builtins(
            ...
            try:
                ...
                for fn in ir:
                    ...
                    args = set(
                        reg for reg, i in fn.env.indexes.items() if i < len(fn.args)
                    )
                    ...
                    for key in sorted(
                        ...
                        pre = ", ".join(
                            sorted(reg.name for reg in analysis_result.before[key])
                        )
                        post = ", ".join(
                            sorted(reg.name for reg in analysis_result.after[key])
                        )
                        actual.append(
                            "%-8s %-23s %s"
