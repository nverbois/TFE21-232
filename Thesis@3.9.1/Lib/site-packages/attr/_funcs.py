...
def _asdict_anything(
    val, filter, dict_factory, retain_collection_types, value_serializer,
):
    """
    ...
    if getattr(val.__class__, "__attrs_attrs__", None) is not None:
        # Attrs class.
        rv = asdict(
            val, True, filter, dict_factory, retain_collection_types, value_serializer,
        )
        ...
        rv = cf(
            [
                _asdict_anything(
                    i, filter, dict_factory, retain_collection_types, value_serializer,
                )
                for i in val
            ]
        )
