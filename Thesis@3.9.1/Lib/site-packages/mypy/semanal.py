...
class SemanticAnalyzer(
    ...
    def analyze_class_typevar_declaration(
        self, base: Type
    ) -> Optional[Tuple[TypeVarList, bool]]:
    ...
    def visit_import(self, i: Import) -> None:
        ...
        for id, as_id in i.ids:
            if as_id is not None:
                ...
                module_public = not self.is_stub_file and self.options.implicit_reexport
                base = id.split(".")[0]
        ...
        if isinstance(node.node, PlaceholderNode):
            if self.final_iteration:
                self.report_missing_module_attribute(
                    module_id, id, imported_id, context
                )
                return
            else:
    ...
    def report_missing_module_attribute(
        self, import_id: str, source_id: str, imported_id: str, context: Node
    ) -> None:
        # Missing attribute.
    ...
    def analyze_namedtuple_assign(self, s: AssignmentStmt) -> bool:
        ...
        lvalue = s.lvalues[0]
        name = lvalue.name
        is_named_tuple, info = self.named_tuple_analyzer.check_namedtuple(
    ...
    def process_type_annotation(self, s: AssignmentStmt) -> None:
        ...
        if s.type:
            ...
            if (
                any(isinstance(lv, NameExpr) and lv.is_inferred_def for lv in s.lvalues)
                and self.type
                and self.type.is_protocol
    ...
    def add_unknown_imported_symbol(
        self, name: str, context: Context, target_name: Optional[str] = None
    ) -> None:
