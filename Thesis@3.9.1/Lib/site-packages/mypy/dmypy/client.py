...
p.add_argument(
    "--log-file", metavar="FILE", type=str, help="Direct daemon stdout/stderr to FILE"
)
p.add_argument(
    "--timeout",
    metavar="TIMEOUT",
...
p.add_argument(
    "--fswatcher-dump-file", help="Collect information about the current file state"
)
...
p.add_argument(
    "--perf-stats-file", help="write performance information to the given file"
)
...
p.add_argument(
    "--remove", metavar="FILE", nargs="*", help="Files to remove from the run"
)
...
def start_server(args: argparse.Namespace, allow_sources: bool = False) -> None:
    ...
    if daemonize(
        start_options, args.status_file, timeout=args.timeout, log_file=args.log_file
    ):
        sys.exit(2)
...
def do_run(args: argparse.Namespace) -> None:
    ...
    if "restart" in response:
        ...
        response = request(
            args.status_file, "run", version=__version__, args=args.flags
        )
...
def do_recheck(args: argparse.Namespace) -> None:
    ...
    if args.remove is not None or args.update is not None:
        response = request(
            args.status_file, "recheck", remove=args.remove, update=args.update
        )
    else:
...
def request(
    ...
    args["terminal_width"] = (
        int(os.getenv("MYPY_FORCE_TERMINAL_WIDTH", "0")) or get_terminal_width()
    )
