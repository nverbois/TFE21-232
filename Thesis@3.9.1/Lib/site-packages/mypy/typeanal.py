...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
    ...
    def visit_unbound_type_nonoptional(
        ...
        if sym is not None:
            ...
            if fullname in nongen_builtins and t.args and not self.allow_unnormalized:
                self.fail(
            ...
            if isinstance(node, TypeAlias):
                ...
                if (
                    ...
                    and len(  # type: ignore[misc]
                        res.args
                    )
                    != len(res.type.type_vars)
    ...
    def analyze_type_with_type_info(
        self, info: TypeInfo, args: List[Type], ctx: Context
    ) -> Type:
...
def get_omitted_any(
    ...
    if disallow_any:
        ...
        any_type = AnyType(
            TypeOfAny.from_omitted_generics, line=typ.line, column=typ.column
        )
    return any_type
...
def fix_instance(
    ...
    if len(t.args) == 0:
        ...
        any_type = get_omitted_any(
            disallow_any, fail, note, t, fullname, unexpanded_type
        )
...
class TypeVariableQuery(TypeQuery[TypeVarList]):
    ...
    def visit_unbound_type(self, t: UnboundType) -> TypeVarList:
        ...
        if (
            node
            and isinstance(node.node, TypeVarExpr)
...
class InstanceFixer(TypeTraverserVisitor):
    ...
    def visit_instance(self, typ: Instance) -> None:
        ...
        if len(typ.args) != len(typ.type.type_vars):
            fix_instance(
                typ, self.fail, self.note, disallow_any=False, use_generic_error=True
            )
