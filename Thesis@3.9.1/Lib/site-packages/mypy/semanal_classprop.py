...
TYPE_PROMOTIONS_PYTHON3.update(
    {"builtins.bytearray": "bytes", "builtins.memoryview": "bytes",}
)
...
def calculate_class_abstract_status(
    ...
    if is_stub_file:
        if (
            typ.declared_metaclass
            and typ.declared_metaclass.type.fullname == "abc.ABCMeta"
        ):
            return
        if typ.is_protocol:
            return
        if abstract and not abstract_in_this_class:
            ...
            report(
                "Class {} has abstract attributes {}".format(typ.fullname, attrs),
                "error",
            )
            report(
...
def add_type_promotion(
    info: TypeInfo, module_names: SymbolTable, options: Options
) -> None:
