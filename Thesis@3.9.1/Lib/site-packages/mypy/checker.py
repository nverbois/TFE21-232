...
FineGrainedDeferredNode = NamedTuple(
    ...
    [("node", FineGrainedDeferredNodeType), ("active_typeinfo", Optional[TypeInfo]),],
)
...
TypeRange = NamedTuple(
    "TypeRange", [("item", Type), ("is_upper_bound", bool),]  # False => precise type
)
...
PartialTypeScope = NamedTuple(
    "PartialTypeScope",
    [("map", Dict[Var, Context]), ("is_function", bool), ("is_local", bool),],
)
...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def visit_block(self, b: Block) -> None:
        ...
        for s in b.body:
            if self.binder.is_unreachable():
                if self.should_report_unreachable_issues() and not self.is_raising_or_empty(
                    s
                ):
        ...
        if self.check_rvalue_count_in_assignment(
            ...
            if not undefined_rvalue:
                ...
                if isinstance(reinferred_rvalue_type, UnionType):
                    ...
                    return
                if (
                    isinstance(reinferred_rvalue_type, AnyType)
                    and self.current_node_deferred
                ):
    ...
    def intersect_instances(
        self, instances: Sequence[Instance], ctx: Context,
    ) -> Optional[Instance]:
        ...
        try:
            cdef, info = self.make_fake_typeinfo(
                curr_module.fullname, full_name, short_name, base_classes,
            )
    ...
    def conditional_type_map_with_intersection(
        self, expr: Expression, expr_type: Type, type_ranges: Optional[List[TypeRange]],
    ) -> Tuple[TypeMap, TypeMap]:
